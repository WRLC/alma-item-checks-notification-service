...
class TestBpNotification:
    ...
    @patch(
        "alma_item_checks_notification_service.blueprints.bp_notification.SessionMaker"
    )
    @patch(
        "alma_item_checks_notification_service.blueprints.bp_notification.NotificationService"
    )
    def test_send_notification_success(
        self, mock_notification_service_class, mock_session_maker_class
    ):
    ...
    def test_send_notification_service_exception(
        self, mock_notification_service_class, mock_session_maker_class
    ):
    ...
    def test_send_notification_session_context_manager(
        self, mock_notification_service_class, mock_session_maker_class
    ):
    ...
    def test_send_notification_session_exception_handling(
        self, mock_notification_service_class, mock_session_maker_class
    ):
    ...
    def test_send_notification_parameter_types(
        self, mock_notification_service_class, mock_session_maker_class
    ):
    ...
    def test_blueprint_configuration(self):
        ...
        if hasattr(send_notification, "_function") and send_notification._function:
            assert (
                send_notification._function.get_function_name() == "send_notification"
            )
    ...
    def test_service_layer_integration(self, db_session):
        ...
        from alma_item_checks_notification_service.services.user_service import (
            UserService,
        )
        ...
        emails = user_process_service.get_user_emails_for_process(
            process.id, user.institution_id
        )
